...
void V4fViewerDialog::OnExportToAvi(wxCommandEvent & event)
{
	...
	if (dlg.ShowModal() == wxID_OK)
	{
		...
		writer.release();
		wchar_t msg[512] = L"";
		_snwprintf(msg, 511, L"Файл сохранен: %s", dlg.GetPath().ToStdWstring().c_str());
		LOG_INFO(msg);
	}
	event.Skip();
}
	...
	try
	{
		if (dataSet)
		{
			...
			if (tmpTmplt.find(L"%ПЕРЕГОН") != std::string::npos)
				tmpTmplt.replace(tmpTmplt.find(L"%ПЕРЕГОН"), 8, string_to_wstring(info.GetAreaPeregon(dataSet->GetFrames()[idxCadre].absCoord + VIDEO_OPTIONS().Value().shiftCoordinateViewer - 1.0, dataSet->GetFrames()[idxCadre].absCoord + VIDEO_OPTIONS().Value().shiftCoordinateViewer + 1.0)).c_str());
