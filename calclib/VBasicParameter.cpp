...
{
	static unsigned long __table__[] = {
	...
	if (s != NULL && len > 0)
	{
		const uint8_t * ucs = static_cast<const uint8_t *>(s); 
		for (size_t i = 0;  i < len;  i ++)
		{
			sum = __table__[(sum ^ ucs[i]) & 0xff] ^ (sum >> 8);
		}
	}
	return sum;
}
...
/// загрузка из файла
bool VBasicParameter::Load(VMainFile & file, uint32_t type, bool copyToMemory)
{
	// файл не загружен
	if (!file.Loaded())
	{
	...
	// параметр в файле не найден, или файл не загружен
	size_t index = file.GetBlockIndex(type);
	if (index == BAD_SIZE)
	{
		this->message = ERRMSG_BLOCK_NOT_FOUND;
		return false;
	}

	// ошибка при маппинге блока данных
	const VDataBlock * block = file.MapBlock(index);
	if (block == NULL) 
	{
	...
	return this->LoadBlock(block, copyToMemory);
}

/// Отцепиться от файла
void VBasicParameter::UnloadFile(VMainFile & file, uint32_t type)
{
	size_t index = file.GetBlockIndex(type);
	if (index == BAD_SIZE) return;
	file.UnmapBlock(index);
}

/// загрузка из удаленного файла
bool VBasicParameter::Load(VRemoteFile & remote, uint32_t type, bool copyToMemory)
{
	if (!remote.Loaded())
	{
		this->message = ERRMSG_FILE_NOT_OPEN;
	...
	// параметр в файле не найден, или файл не загружен
	const VDataBlock * block = remote.GetBlock(type);
	if (block == NULL)
	{
		this->message = ERRMSG_BLOCK_NOT_FOUND;
		return false;
...
void VBasicParameter::UnloadFile(VRemoteFile & remote, uint32_t type)
{
	remote.FreeBlock(type);
}

// загрузка параметра из блока данных
bool VBasicParameter::LoadBlock(const VDataBlock * dataBlock, bool copyToMemory)
{
	this->Unload();
	if (dataBlock == NULL || dataBlock->Data == NULL || 
		dataBlock->Header == NULL || dataBlock->Header->DataSize == 0)
	{
	...
	// неразобранные флаги
	this->flags = dataBlock->Header->Flags;

	// тип данных
	this->dataType = dataBlock->Header->Type;

	// время создания блока
	if (this->flags & PHO_CREATE_TIME)
		this->createTime = dataBlock->Header->CreateTime;
	else 
		this->createTime = time(NULL);

	// время обновления блока
	if (this->flags & PHO_UPDATE_TIME)
		this->updateTime = dataBlock->Header->UpdateTime;
	else 
		this->updateTime = time(NULL);

	// версия
	if (this->flags & PHO_REC_VER)
		this->version = dataBlock->Header->RecVer;
	else
		this->version = 0;

	// размер данных
	this->dataSize = dataBlock->Header->DataSize;

	// блок данных
	if (copyToMemory)
	{
		// перегрузка данных в память
		this->dataPtr = (uint8_t*) ::GlobalAlloc(GPTR, this->dataSize);
		if (this->dataPtr == NULL)
		{
			this->message = ERRMSG_MEMORY_ALLOC_ERROR;
			return false;
		}

		::CopyMemory(this->dataPtr, dataBlock->Data, this->dataSize);
		this->isAlloc = true;
	}
	else
	{
		// 
		this->dataPtr = dataBlock->Data;
		this->isAlloc = false;
	}
	this->crc = GetCrc32(this->dataPtr, this->dataSize);

	// число записей и размер каждой (если есть)
	if (this->flags & PHO_REC_COUNT || this->flags & PHO_REC_SIZE)
	{
		this->recCount = dataBlock->Header->RecCount;
		this->recSize = dataBlock->Header->RecSize;

		// тип С
		if (this->flags & PHO_C_TYPE)
		{
			this->cType = dataBlock->Header->CType;
		}
		else
		{
			this->cType = VCT_USER;
	...
	// размер записи не указан - например, история операций
	else if (this->flags & PHO_REC_COUNT)
	{
		this->recCount = dataBlock->Header->RecCount;
		this->recSize = 0;
		this->cType = VCT_USER;
	...
	else // одна запись размером с блок
	{
		this->recCount = 1;
		this->recSize = this->dataSize;
		this->cType = VCT_USER;
	}
...
bool VBasicParameter::Create(uint32_t dataType, uint32_t cType, uint32_t recCount, uint32_t recSize, uint32_t version)
{
	this->Unload();

	this->dataPtr = (uint8_t*) ::GlobalAlloc(GPTR, recCount * recSize);
	if (this->dataPtr == NULL)
	{
		this->message = ERRMSG_MEMORY_ALLOC_ERROR;
		return false;
	...
	this->dataSize = recCount * recSize;
...
// Сохраняем в файл
bool VBasicParameter::Save(VMainFile & file, uint32_t recCount, uint32_t recSize)
{
	VDataBlockHeader hdr = {0};
	hdr.HdrSize = sizeof(hdr);
	strncpy(hdr.cSign, "DATABLK", 7);
	hdr.Flags = PHO_CREATE_TIME|PHO_UPDATE_TIME|PHO_REC_VER;
	hdr.Type = this->dataType;
	hdr.CreateTime = this->createTime;
	hdr.UpdateTime = this->updateTime;
	hdr.RecVer = this->version;
	hdr.DataSize = this->dataSize;
	hdr.RecCount = recCount;
	hdr.RecSize = recSize;

	if (recCount > 0)
	{
		hdr.Flags |= PHO_REC_COUNT;
	}

	if (recSize > 0)
	{
		hdr.Flags |= PHO_REC_SIZE;
	}

	if (this->cType != VCT_USER)
	{
		hdr.CType = this->cType;
		hdr.Flags |= PHO_C_TYPE;
	}

	size_t idx = file.GetBlockIndex(this->dataType);
	if (idx != BAD_SIZE)
	{
	...
	try
	{
		file.AddBlock(&hdr, this->dataPtr);
	}
	catch(...)
	{
...
// выгрузка параметра
void VBasicParameter::Unload()
{
	// освобождаем выделенную память
	if (this->isAlloc && this->dataPtr != NULL)
	{
		::GlobalFree(this->dataPtr);
	}
	...
	this->index = 0;
	this->version = 0;
	this->createTime = VTime(0);
	this->updateTime = VTime(0);
	this->recCount = 0;
	this->recSize = 0;
	this->cType = VCT_UNKNOWN;
	this->flags = 0;
